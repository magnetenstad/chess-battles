...
class AutoChessTowerDefense:
    def __init__(self) -> None:
        pygame.init()
        pygame.display.set_caption("Auto Chess Tower Defense")
    ...
    def reset(self) -> None:
        ...
        self.status_text = "Auto mode: buy pieces and place them. Keep health above 0."
    ...
    def spawn_black_pawn(self) -> bool:
    ...
    def move_black_pawns(self) -> tuple[int, int]:
        moved = 0
        king_captures = 0
        for y in range(7, -1, -1):
            for x in range(8):
                ...
                if captures:
                    ...
                    if taken.kind == "K":
                        # King capture deals damage but does not also count as a breach.
    ...
    def black_turn(self) -> None:
        moved, king_captures = self.move_black_pawns()

        if king_captures > 0:
            damage = KING_CAPTURE_DAMAGE * king_captures
            label = "Black captured your king." if king_captures == 1 else f"Black captured your king {king_captures} times."
        ...
        spawned = self.spawn_black_pawn()

        if self.game_over:
            return

        if king_captures == 0:
            spawn_text = "spawned 1 pawn" if spawned else "spawn blocked"
            self.status_text = f"Black turn: {moved} pawn moves, {spawn_text}."
    ...
    def draw_panel(self) -> None:
        ...
        instruction_lines = [
            ...
            "- Each turn is exactly 1 seconds.",
            "- White auto-moves exactly one piece each turn.",
            "- Black pawns move automatically and spawn.",
    ...
    def draw_status_bar(self) -> None:
        ...
        title = self.title_font.render("Auto Chess Tower Defense", True, TEXT_COLOR)
